...
import numpy as np
import base64
from billi.constants import CANDLE_SOURCE_MAPPING
from billi.constants import TIMEFRAME_PRIORITY
from billi.constants import SUPPORTED_COLORS
from billi.enums import timeframes

CACHED_CONFIG = dict()


def app_currency() -> str:
    from billi.info import exchange_info
...
def get_strategy_class(strategy_name: str):
    ...
    # main framework
        ...
        strategy_dir = f'billi.strategies.{strategy_name}.{strategy_name}'
...
def opposite_type(t: str) -> str:
    from billi.enums import trade_types
...
def quote_asset(symbol: str) -> str:
    try:
        ...
        from billi.exceptions import InvalidRoutes
...
def side_to_type(s: str) -> str:
    from billi.enums import trade_types, sides
...
def timeframe_to_one_minutes(timeframe: str) -> int:
    from billi.utils import timeframe_to_one_minutes
...
def validate_cwd() -> None:
    ...
    """
    if not is_billi_project():
        print(
            color(
                'Current directory is not a Billi project. You must run commands from the root of a Billi project.',
                'red'
            )
        )
        os._exit(1)
