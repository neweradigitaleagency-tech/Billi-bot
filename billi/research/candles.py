...
from billi import factories
import billi.helpers as jh
from billi.services.candle_service import get_candles_from_db as _get_candles
...
def store_candles(candles: np.ndarray, exchange: str, symbol: str) -> None:
    ...
    """
    from billi.modes.import_candles_mode import store_candles_list as store_candles_from_list
    import billi.helpers as jh

    # check if .env file exists
    if not jh.is_unit_testing() and not jh.is_billi_project():
        raise FileNotFoundError(
            'Invalid directory: ".env" file not found. To use Jesse inside notebooks, create notebooks inside the root of a Jesse project.'
        )
