...
from billi.info import exchange_info, jesse_supported_timeframes, JESSE_API_URL
...
def get_general_info(has_live=False) -> dict:
    ...
    if access_token:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                raise Exception(
                    f"Failed to get user info from billi API: {error_message}"
                )
    ...
    try:
        ...
        if response.status_code == 200 and 'application/json' in response.headers.get('Content-Type', ''):
            ...
            raise ValueError("Invalid response from billi API")
