...
@click.group()
@click.version_option(get_version("billi"))
def cli() -> None:
    """CLI entrypoint for Billi Bot."""
    pass
...
def install_live(strict: bool) -> None:
    ...
    from billi.services.installer import install
...
@cli.command()
def run() -> None:
    """Start the Billi Bot application server."""
...
╚█████╔╝███████╗███████║███████║███████╗
 ╚════╝ ╚══════╝╚══════╝╚══════╝╚══════╝
    ...
    """
    version = get_version("billi")
    print(welcome_message)

    version_line = (
        click.style("  Billi Bot ", fg="white")
    ...
    from billi.services.migrator import run as run_migrations
    import peewee
    ...
    try:
        from billi.services.lsp import install_lsp_server
    ...
    global HOST, PORT
    from billi.services.env import ENV_VALUES
    ...
    # run the lsp server
    try:
        from billi.services.lsp import run_lsp_server
    ...
    # run the mcp server
    try:
        from billi.mcp import run_mcp_server

        run_mcp_server(billi_host=HOST, billi_port=PORT)
    except Exception as e:
